 ...
 */
function isProjectOpenInIntelliJ(projectPath: string): boolean {
  try {
    ...
    if (platform === 'darwin') { // macOS
      ...
      try {
        ...
        const result = execSync(`osascript -e '${appleScriptCommand.replace(/'/g, "'\\''")}' 2>/dev/null`).toString().trim();
        return result === 'true';
      } catch (error) {
      ...
      try {
        const projectName = path.basename(projectPath);
        const result = execSync(`wmctrl -l | grep -i "${projectName}" | grep -i "IntelliJ" || echo "Not found"`).toString();
 ...
 */
function jumpToIntelliJ(filePath: string, lineNumber: number): void {
  // 检查文件是否存在
    ...
    if (platform === 'darwin') { // macOS
      ...
      if (!isProjectOpen && projectRoot) {
        try {
          ...
          if (ideaScript) {
            // 先打开项目
            execSync(`"${ideaScript}" "${projectRoot}"`);
            ...
            // 使用open命令打开项目
            execSync(`open -a "${ideaAppPath}" "${projectRoot}"`);
            ...
            execSync(`osascript -e '${appleScriptCommand.replace(/'/g, "'\\''")}' &`);
            vscode.window.showInformationMessage(`已请求IntelliJ IDEA打开项目并跳转到文件: ${absolutePath}:${lineNumber}`);
            return;
          }
        } catch (error) {
      ...
      if (ideaScript) {
        ...
        const process = spawn(ideaScript, args, {
          detached: true,
          stdio: 'ignore'
        });
      ...
      if (!isProjectOpen && projectRoot) {
        ...
        if (!ideaPath) {
          ...
          try {
            ...
            ideaPath = execSync(regQueryCmd).toString().trim();
          } catch (error) {
            // 注册表查询失败，使用默认路径
            ideaPath = 'idea64.exe';
          }
        ...
        try {
          ...
          const processProject = spawn(ideaPath, [projectRoot], {
            detached: true,
            stdio: 'ignore'
          });
          ...
          try {
            ...
            // 回退方法：尝试直接打开文件
            const processFile = spawn(ideaPath, ['--line', lineNumber.toString(), absolutePath], {
              detached: true,
              stdio: 'ignore'
            });
            processFile.unref();
      ...
      if (!isProjectOpen && projectRoot) {
        ...
        if (!ideaPath) {
          ...
          for (const globPattern of userPaths) {
            try {
              // 使用find命令查找匹配的文件
              const foundPath = execSync(`find ${globPattern.replace(/\*/g, '\\*')} -type f 2>/dev/null | head -1`).toString().trim();
 ...
 */
function getCurrentFileInfo(): { filePath: string; lineNumber: number } | null {
